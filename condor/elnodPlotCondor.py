...
# script used on OSG to generate plots for elnod
...
def ElnodIQPhaseAngle(request):
    ...
    plt.tight_layout()
    plt.savefig(str(request['observation'])+'_'+'ElnodIQPhaseAngle_'+obstype+'.png', bbox_inches='tight',dpi=100)
    plt.close()
def ElnodSNSlopesHistogram(request):
    ...
    plt.tight_layout()
    plt.savefig(str(request['observation'])+'_'+'ElnodSNSlopesHistogram_'+obstype+'.png', bbox_inches='tight',dpi=100)
    plt.close()
'''
