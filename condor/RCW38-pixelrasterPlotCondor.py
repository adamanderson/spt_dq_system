...
# script used on OSG to generate plots for RCW38-pixelraster
# Pass in the needed parameters using argparse
obstype='RCW38-pixelraster'
P = ap.ArgumentParser(description='Various Plots corresponding to source',
...
dpi_setting=200
tar = tarfile.open(year+'_'+obstype+'.tar.gz', "r:gz")
tar.extractall()
tar.close()
tar = tarfile.open(year+'_'+obstype+'BolometerProperties.tar.gz', "r:gz")
tar.extractall()
tar.close()

# obsp keeps track of a list of parameter that we want to plot for
obsp=["PointingOffsetX","PointingOffsetY","RCW38FluxCalibration","RCW38IntegralFlux"]
...
def StackedHistogram(request):
    ...
    if np.isfinite(plotrange[0]) and np.isfinite(plotrange[1]):
        ...
        # dealing with the case when all data is empty
        if sum([len(l) for l in bindata])==0:
            return
    ...
    for i in range(len(bands)):
        ...
        if len(xdata)>0 and len(ydata)>0 and len(bindata)>0:
            ...
            if np.isfinite(pltrange[0]) and np.isfinite(pltrange[1]):
                ...
                plt.savefig(str(obsid)+'_'+str(bands[i]/core.G3Units.GHz)+'GHz_'+'focalplane_'+obstype+'_' + str(varname)+'.png',bbox_inches='tight',dpi=dpi_setting)
...
def RCW38IntegralFlux(request):
    ...
    plt.title('RCW38IntegralFlux' + ' at time ' + str(request['observation']))
    plt.savefig(str(request['observation'])+'_'+'RCW38IntegralFlux_'+obstype+'.png', bbox_inches='tight',dpi=dpi_setting)
    plt.close()
...
def RCW38FluxCalibration(request):
    ...
    plt.title('RCW38FluxCalibration' + ' at time ' + str(request['observation']))
    plt.savefig(str(request['observation'])+'_'+'RCW38FluxCalibration_'+obstype+'.png', bbox_inches='tight',dpi=dpi_setting)
    plt.close()
