...
def NET1to2HzHistogram(request):
    try:
    ...
    try:
        cal_dict = {}
        for band in bands:
            cal_dict[band] = np.array([data[0]['NET_1.0Hz_to_2.0Hz'][bolo] \
                                       for bolo in data[0]['NET_1.0Hz_to_2.0Hz'].keys() \
        ...
        return "NET_1.0Hz_to_2.0Hz does not exist for this observation."
    ...
    plt.title('NET in 1-2Hz for observation {}\n'.format(request['observation']))
    plt.tight_layout()
    return fig
