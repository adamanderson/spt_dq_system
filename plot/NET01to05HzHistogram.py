...
def NET01to05HzHistogram(request):
    try:
    ...
    try:
        cal_dict = {}
        for band in bands:
            cal_dict[band] = np.array([data[0]['NET_0.1Hz_to_0.5Hz'][bolo] \
                                       for bolo in data[0]['NET_0.1Hz_to_0.5Hz'].keys() \
        ...
        return "NET_0.1Hz_to_0.5Hz does not exist for this observation."
    ...
    plt.title('NET in 0.1-0.5Hz for observation {}\n'.format(request['observation']))
    plt.tight_layout()
    return fig
