...
conn_transferdb = sqlite3.connect('transfer.db')
db = sdb.TransferDatabase('/spt/data/transfer_database/transfer.txt', user='adama', read_only=True)
dbdata = db.data
dbdata['modified'] = (dbdata['modified']).apply(lambda d: pd.to_datetime(str(d)))
dbdata.to_sql('transfer_db', conn_transferdb, if_exists='append')
conn_transferdb.commit()
conn_transferdb.close()
...
while jbatch*n_files_per_worker < len(plot_queue):
    ...
    tar_path = args.plotdir
    tar_input_fname = '{}_{}_input_data.tar'.format(timestamp, jbatch)
    tar_cal_fname = '{}_{}_online_cal.tar'.format(timestamp, jbatch)
    tar_input = tarfile.open('{}/{}'.format(tar_path, tar_input_fname), 'w')
    tar_cal = tarfile.open('{}/{}'.format(tar_path, tar_cal_fname), 'w')
    ...
    condor_configs = {
        'script': 'plotter.py',
        'args': ['{}'.format(pathlist_file), '{}_{}_plots'.format(timestamp, jbatch)],
        ...
        'input_files': ['{}/{}'.format(tar_path,tar_input_fname),
                        '{}/{}'.format(tar_path,tar_cal_fname)],
